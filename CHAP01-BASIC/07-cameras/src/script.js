 ...
 * 
 * TrackballControls
 * TrackballControls is just like OrbitControls but there are no limits in terms of vertical angle. You can keep rotating and do spins with the camera
 * even if the scene gets upside down.
 * 
 * TransformControls
 * TransformControls has nothing to do with the camera. You can use it to add a gizmo to an object to move that object. its like an editor
 * 
 * DragControls
 * Just like the TransformControls, DragControls has nothing to do with the camera. You can use it to move objects on a plane facing the camera by
 * drag and dropping them.
 */
...
const tick = () =>
{
   ...
   // mesh.rotation.y = elapsedTime;
    ...
    // In order to work properly, the Damping controls also needs to be updated on each frame by calling controls.update()
    controls.update()
